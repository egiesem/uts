...
export default class BookmarkView extends Component {
  ...
  render() {
    ...
    return (<ScrollView ref={'registerFormC'} {...this.props}>
                <ThemeProvider uiTheme={uiTheme}>
        <Container>
          <Toolbar
            leftElement="close"
            onLeftElementPress={() => this.props.navigation.navigate('Info')}
            centerElement="Registrasi"
          />
        <View key={'messages'}>
          {this.renderMessages()}
        </View>
        ...
        <View style={{flex:1,flexDirection:'row',alignItems:'flex-start',margin:8}}>
          <Text style={{fontSize:17}}>{'Sudah punya akun? '}</Text>
          <TouchableOpacity activeOpacity={0.7} onPress={() => this.props.navigation.navigate('Info')}>
